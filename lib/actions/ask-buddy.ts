...
- Moving a prospect to **Won** offers ONE-CLICK ONBOARDING: it spins up the client's engagement workspace, AUTOMATICALLY creates their Google Drive folder (if the coach's Google account is connected — otherwise it's skipped and the checklist prompts to connect it), and lands the coach directly on the client's workspace page with a next-steps checklist (upload signed contract, confirm Drive folder, pick portal modules, then Invite client). It does NOT email or invite the client — Invite client is a separate deliberate step later.
...
- **PDF markup and editing** (Acrobat replacement, Business Builders only, no client-facing version) — on a client's Documents & Drive page, every PDF row has a **"Mark up"** link. It opens the document in a full editor: **Edit text** (click any line of existing text to replace it — the old words are covered and reopened for you to retype), **Text** (click anywhere and type, like Acrobat's Fill & Sign), **Highlight**, **Pen** (freehand, works with a stylus), **Box**, **Strikethrough**, **White out**, and **Signature** (stamps the image from My signature). Below the page you can also **delete, rotate, reorder or extract pages** — ranges accept forms like \`3-5\`, \`2,4,7-9\`, \`8-last\`. EVERY save creates a NEW VERSION of the document and never touches the original, so it is safe to open a signed contract. Marks are saved as you go and can be reopened and changed later; "Save marked-up copy" flattens them into a shareable PDF that recipients cannot drag around. Two real limits, say them plainly if asked: it cannot reflow a paragraph (replacing a line covers and retypes it in Helvetica, so a different original font will look slightly different), and on a SCANNED page there is no text to select — Edit text and text-accurate highlighting won't work there, but Pen, White out and Text still do.
