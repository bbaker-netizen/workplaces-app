...
**WHOSE CLIENTS YOU SEE (important — this changed):** Everyone, master admin included, now lands on THEIR OWN BOOK by default — the clients they own, plus any lead nobody has claimed yet. There's a **Just mine / All clients** toggle on the Console home and the Pipeline, but it is only offered to people allowed the whole practice (the master admin, and any Business Builder given all-clients access). A standard Business Builder restricted to their own clients does not get the toggle, and cannot reach another Builder's client by pasting a URL either — it's a real boundary, not just a filtered list. Which clients someone owns is driven by the **Owner** field on the lead, not by a separate list.
- Action items (/business-builder/action-items) — the ONE follow-through list. Small commitments measured in hours/days AND the nine big documents, which are now action items carrying a document type rather than a separate Deliverables module (that module is gone). **Everything a session produces lives on that meeting's workspace.** Open the client's Meetings page (/business-builder/engagements/<id>/meetings) and hit **Open workspace** on a recorded meeting. One button there — **Draft from this meeting** — reads the WHOLE transcript (not just the Fireflies highlights) and writes up BOTH the commitments people made AND any of the nine documents the session actually called for. There is no type picker: it decides from what was said. Because that takes longer than a web request it runs in the BACKGROUND — to-dos land in about a minute, documents take a few minutes. **The page watches for them and shows them on its own; there is no need to refresh, and the button says so while it works.** Everything arrives on the same page under **Needs your review** as DRAFTS: edit the wording AND the detail (the description is editable right there — press "+ Add detail" if the item has none), set an owner (you, Jen, or someone on the client's team) and a due date, change a to-do into one of the nine documents or back, then **Publish**. Nothing is visible to the client until published. **Add something the transcript missed** on the same page creates an item by hand. **Tick the checkbox on several items and a "Delete selected" bar appears** — for clearing a drafting run you don't want without twenty separate confirms; there's a Select all per section, and the confirm counts separately anything already published and visible to the client.

**The two client-facing controls on the workspace do DIFFERENT things and are labelled to say so** (they used to both read "…to client" and were genuinely confusing):
- **"Email the recap"** (top, on the recap panel) — emails the written write-up to the client's portal contacts and files it on their portal thread. The button states how many people it will actually reach BEFORE you press it; at zero it reads "File it" instead. Edit the recap first with **Edit** — it is a proper rich-text editor, not markdown source. **"Discard"** bins the write-up for a session that doesn't warrant one; it does NOT touch the transcript, no new recap will be drafted for that session afterwards, and it only works while the recap is still a draft (a sent recap is the record of what the client was told).
- **"Show in their portal"** (bottom, on the transcript panel) — makes the word-for-word transcript readable by EVERYONE on that client's team. It emails nobody. Nothing is readable until you press it, and "Hide from their portal" takes it back.
These are independent: a client can get the transcript with no written recap, or a recap with no transcript.
...
- On a client who is already up and running (they hold a real portal org, or a session has already been held), the panel **collapses to a single line** with a "Show" control — because onboarding is a few days out of an engagement that runs for years, and a permanent "not ready" box against an established client reads as outstanding work. Nothing is hidden; it can always be expanded, and the same four checks apply when it is: the button is gated for every client, established or not, because the checks guard real sends. On an established client the box explains itself differently — those items are only needed to RUN the welcome sequence, so if the client is already going, nothing there is outstanding and it can be left alone.
