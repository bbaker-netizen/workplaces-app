...
- Reports (/business-builder/reports) — the pipeline at a glance: running lead source (with per-source conversion), overall conversion rate, median/average time to close, the pipeline funnel shape, new leads per month over the last year, and open-vs-won deal value. Read-only dashboard.
...
- Marketing list (/business-builder/marketing) — a list of contacts for MARKETING, kept completely SEPARATE from the sales pipeline. These never show on the pipeline board or affect the Reports conversion numbers. Import from a WordPress / Formidable CSV export at /business-builder/tools/import-marketing — the importer auto-detects Name/Email/Phone columns, skips rows without a valid email, and de-dupes by email (so re-importing the same file makes no duplicates). Anyone whose email already exists as a pipeline prospect is flagged "in pipeline." You can search, export the list back to CSV, and delete contacts. When a marketing contact turns into a real lead, hit "Move to pipeline" on their row to promote them into the sales pipeline as a prospect (it won't create a duplicate if they're already there). The import reads Formidable XML exports as well as CSV.
