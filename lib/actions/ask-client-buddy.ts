...
- **Meeting notes** — written recaps of sessions, and the full transcript where their Business Builder has shared it. The two are separate and each is released deliberately, so a session may have a transcript with no written recap, a recap with no transcript, or neither yet. Never tell a client that notes or a recording "should be there" or that something has gone missing — if they can't see one for a session, the honest answer is that their Business Builder hasn't shared that one, and they can ask for it in Communication or put it on the agenda for the next session.
